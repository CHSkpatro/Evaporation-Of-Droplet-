...
h0 = 0.0875  # Maximum height of the droplet (can be adjusted as needed)
...
for iter_num in range(iter_max + 1):
    ...
    for i in range(1, N):
        ...
        if i > int((R / R_max) / del_r):
            C_new[0, i:] = C_new[1, i:]  # No penetration condition
    for j in range(1, M):
        if j > int((h0 / Z_max) / del_z):
            C_new[j:, 0] = C_new[j:, 1]  # Symmetry condition
...
plt.savefig('RMS_Error_vs_Iteration_Number_24.jpg')
plt.show()

# Finding boundary positions for cap surface
boundary_points = []
for i in range(N):
    if r[i] <= R / R_max:
        boundary_indices = np.where(z <= h(r[i], R / R_max, theta))[0]
...
# Mask the interior region but keep boundary points visible
C_masked = np.ma.masked_where(interior == 1, C_new)  # Mask only the interior points
# Note: boundary points should not be masked

# Create a discrete colormap
cmap = mcolors.ListedColormap(['blue', 'green', 'yellow', 'orange', 'red',
                               'purple', 'pink', 'brown', 'gray', 'black',
                               'cyan', 'magenta', 'lime', 'maroon', 'navy'])

# Plot the final concentration contour for the exterior region
plt.figure(figsize=(10, 6))
num_levels = 50  # Increase the number of contour levels
contour = plt.contourf(R_grid, Z_grid, C_masked, levels=num_levels, cmap=cmap, alpha=0.8)
...
plt.ylim(0, 1)
plt.legend()
plt.savefig('Sessile_droplet_profile_and_Concentration_Contour_23.jpg')
plt.show()
...
ax.set_xlim(0, r_max)
ax.set_ylim(0, z_max)
plt.savefig('Concentration_gradient_vector_field_at_boundary_scale_1_23.jpg')
plt.show()
...
for scale, filename in [(0.1, '0.1'), (0.01, '0.01'), (2, '2')]:
    ...
    ax.set_xlim(0, r_max)
    ax.set_ylim(0, z_max)
    plt.savefig(f'Concentration_gradient_vector_field_at_boundary_scale_{filename}_23.jpg')
    plt.show()
...
# Save to CSV
gradient_df.to_csv('gradient_vectors_23.csv', index=False)
