...
C_df.to_csv('concentration_values_25.csv', index=False)
...
plt.savefig('RMS_Error_vs_Iteration_Number_25.jpg')
plt.show()

# Finding boundary positions within the droplet cap surface
boundary_points = []
for i in range(N):
    if r[i] <= R / R_max:
        boundary_indices = np.where(z <= h(r[i], R / R_max, theta))[0]
...
plt.title('Spherical Droplet Profile and Concentration Contour')
plt.ylim(0, 1)
plt.legend()
plt.savefig('Spherical_droplet_profile_and_Concentration_Contour_25.jpg')
plt.show()
...
ax.set_xlim(0, r_max)
ax.set_ylim(0, z_max)
plt.savefig('Spherical_Concentration_gradient_vector_field_at_boundary_scale_1_25.jpg')
plt.show()
...
# Save to CSV
gradient_df.to_csv('spherical_gradient_vectors_25.csv', index=False)
