...
plt.savefig('RMS_Error_vs_Iteration_Number_13:57.jpg')
plt.show()
...
contour = plt.contourf(R_grid, Z_grid, C_masked, levels=np.linspace(C_masked.min(), C_masked.max(),300), cmap='coolwarm', alpha=0.6)
...
plt.ylim(0, 1)
plt.legend()
plt.savefig('Sessile_droplet_profile_and_Concentration_Contour_13:57.jpg')
plt.show()
...
Q = ax.quiver(r[boundary_positions[:, 1]], z[boundary_positions[:, 0]], 
              C_grad_r, C_grad_z, 
              magnitude_boundary, scale=10**-1, cmap='rainbow')
...
ax.set_xlim(0, r_max)
ax.set_ylim(0, z_max)
plt.savefig('Concentration_gradient_vector_field_at_boundary_13:57.jpg')
plt.show()
...
Q = ax.quiver(r[boundary_positions[:, 1]], z[boundary_positions[:, 0]], 
              C_grad_r, C_grad_z, 
              magnitude_boundary, scale=2, cmap='rainbow')
fig.colorbar(Q, ax=ax)
ax.set_xlabel('r in normalized form')
ax.set_ylabel('z in normalized form')
ax.set_title('Concentration Gradient Vector Field at Droplet Surface')
ax.set_xlim(0, r_max)
ax.set_ylim(0, z_max)
plt.savefig('Concentration_gradiet_vector_field_at_boundary_13:57.jpg')
plt.show()
...
Q = ax.quiver(r[boundary_positions[:, 1]], z[boundary_positions[:, 0]], 
              C_grad_r, C_grad_z, 
              magnitude_boundary, scale=3, cmap='rainbow')
fig.colorbar(Q, ax=ax)
ax.set_xlabel('r in normalized form')
ax.set_ylabel('z in normalized form')
ax.set_title('Concentration Gradient Vector Field at Droplet Surface')
...
# Save to CSV
gradient_df.to_csv('gradient_vectors_13:57.csv', index=False)
